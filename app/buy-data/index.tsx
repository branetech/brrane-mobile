...
const toArray = (value: unknown): DataPlan[] => {
  if (Array.isArray(value)) return value as DataPlan[];
  const v = value as any;
  if (Array.isArray(v?.data?.variations)) return v.data.variations;
  if (Array.isArray(v?.data?.varations)) return v.data.varations;
  if (Array.isArray(v?.variations)) return v.variations;
  return [];
};
...
export default function BuyDataScreen() {
  ...
  const fetchPlans = useCallback(async (network: Network) => {
  ...
  const handleSubmit = async () => {
    ...
    setPinVisible(false);
    showAppLoader({ message: "Processing data purchase…" });
    try {
      await BaseRequest.post(MOBILE_SERVICE.BUY_DATA, {
                ...
                >
                  {NETWORKS.map((n) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={n.id}
                        onPress={() => handleNetworkSelect(n.id)}
                        style={[
                          styles.networkPill,
                          {
